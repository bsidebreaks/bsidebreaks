...
export default function GeneratePage() {
  ...
  const generateTrips = useCallback(async (from?: string | null, to?: string | null) => {
    if (!from || !to) return;
    setLoading(true);
    setError(null);
    setRecommendations([]);
    ...
    try {
      ...
      const recsRes = await fetch('/api/recommendations', {
        ...
        body: JSON.stringify({ musicalDNA: dnaData.musicalDNA, from, to }),
      });
  ...
  useEffect(() => {
    ...
    void generateTrips(from, to);
  }, [status, from, to, generateTrips]);
          ...
          <Card className="w-full">
            ...
            <CardFooter>
              {spotifyExpired ? (
                ...
                <Button className="w-full" onClick={() => void generateTrips(from, to)}>
        ...
        {!loading && !error && recommendations.length > 0 && (
          <div className="flex w-full flex-col items-center gap-6">
            ...
            <Button
              ...
              onClick={() => void generateTrips(from, to)}
            >
              <RotateCcw className="size-4" />
              Generate again
            </Button>
